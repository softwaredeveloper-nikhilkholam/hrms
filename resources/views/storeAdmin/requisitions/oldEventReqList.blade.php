            ...
            <div class="card-header">
                <div class="col-lg-12">
                    <div class="d-flex flex-wrap align-items-center justify-content-between">
                        <div  class="col-lg-4"><b style="color:red;">Old Event Requisition List  <b style="color:blue;">({{$reqCount}})</b></b></div>
                        <div  class="col-lg-8 text-right">
                            ...
                            <a href="/requisitions/oldEventReqList" class="btn mb-1 btn-success">Old Event List</a>                             
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        @if(count($requisitions))
                            <div class="table-responsive">
                                <table id="datatable" data-page-length='25' class="table table-bordered data-table table-striped" style="font-size:12px;" width="auto">
                                    <thead>
                                        <tr class="ligth">
                                            <th style="font-size:14px;border">No</th>
                                            <th style="font-size:14px;">Req. Date</th>           
                                            <th style="font-size:14px;">Req. No.</th>
                                            <th style="font-size:14px;">Branch</th>
                                            <th style="font-size:14px;">Department</th>
                                            <th style="font-size:14px;" width="30%">Requisition For</th>
                                            <th style="font-size:14px;">Sevirity</th>
                                            <th style="font-size:14px;">Status</th>
                                            <th style="font-size:14px;">Action<?php $i=1; ?></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($requisitions as $row)
                                            ...
                                            @else   
                                                ...
                                                @endif
                                            @endif
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{$i++}}
                                                    @if($row->viewedStatus == 0)
                                                ...
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{date('d-M', strtotime($row->requisitionDate))}}</td>
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{$row->requisitionNo}}</td>
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{$row->branchName}}</td>
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{$row->departmentName}}</td>
                                                <td style="padding: 0px 17px !important;font-size:14px;" class="text-left">{{ucwords(strtolower($row->requisitionFor))}}</td>
                                                <td style="padding: 0px 17px !important;font-size:14px;">{{($row->sevirity == 1)?'Normal':(($row->sevirity == 2)?'Urgent':'Very Urgent')}}</td>
