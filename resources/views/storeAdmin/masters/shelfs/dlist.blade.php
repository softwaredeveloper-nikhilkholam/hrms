                ...
                <div class="card-header">
                    <div class="col-lg-12">
                        <div class="d-flex flex-wrap align-items-center justify-content-between">
                            <div  class="col-lg-7"><b style="color:red;">Deactive Shelf List</b></div>
                            <div  class="col-lg-5">
                                <a href="/shelf/create" class="btn mb-1 btn-primary">Add</a>
                                <a href="/shelf/dlist" class="btn mb-1 btn-primary">
                                    Deactive List <span class="badge badge-danger ml-2">{{count($dShelfs)}}</span>
                                </a>
                                ...
                                @if(count($dShelfs))
                                    <table data-page-length='25' class="table table-bordered data-table mb-0 tbl-server-info">
                                        ...
                                        <tbody class="ligth-body">
                                            @foreach($dShelfs as $row)
                                                <tr>
                                                    <td style="padding: 0px 17px !important;">{{$i++}}</td>
                                                    <td style="padding: 0px 17px !important;">H-{{$row->hallName}}</td>
                                                    <td style="padding: 0px 17px !important;">R-{{$row->rackName}}</td>
                                                    <td style="padding: 0px 17px !important;">S-{{$row->name}}</td>
                                                    ...
                                                    <td style="padding: 0px 17px !important;">
                                                        <div class="d-flex align-items-center list-action">
                                                            <a class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"
                                                                href="/shelf/{{$row->id}}/edit"><i class="ri-pencil-line mr-0"></i></a>
                                                            <a class="badge bg-warning mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Activate"
                                                                href="/shelf/{{$row->id}}/activate"><i class="fa fa-check"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
