                ...
                <div class="col-lg-12">
                    <div class="d-flex flex-wrap align-items-center justify-content-between">
                        ...
                        <div  class="col-lg-6">
                            ...
                            <a href="/eventRequisitions" class="btn mb-1 btn-success">Pending List</a>     
                    ...
                    <div class="col-lg-12">
                        <div class="table-responsive">
                            @if(count($requisitions))
                                <table id="datatable" data-page-length='25' class="table table-bordered data-table table-striped" style="font-size:12px;" width="100%">
                                    <thead>
                                        <tr class="ligth">
                                            ...
                                            <th style="padding: 10px 17px !important;font-size:12px;" width="4%">Sevirity</th>
                                            <th style="padding: 10px 17px !important;font-size:12px;" width="10%">Updated</th>
                                            <th style="padding: 10px 17px !important;font-size:12px;" width="5%">Action<?php $i=1; ?></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($requisitions as $row)
                                            ...
                                            @else   
                                                ...
                                                @endif
                                            @endif
                                                <td style="padding: 0px 10px !important;">{{$i++}}<br>
                                                    @if($row->viewedStatus == 0)
                                                ...
                                                <td style="padding: 0px 10px !important;">{{date('d-M', strtotime($row->requisitionDate))}}</td>
                                                <td style="padding: 0px 10px !important;">{{$row->requisitionNo}}</td>
                                                <td style="padding: 0px 10px !important;">{{$row->branchName}}</td>
                                                <td style="padding: 0px 10px !important;">{{$row->departmentName}}</td>
                                                <td style="padding: 0px 10px !important;" class="text-left">{{ucwords(strtolower($row->requisitionFor))}}<br><b style="font-size:0.1px;color:black;">{{$util->getRequisitionProducts($row->id)}}</b></td>
                                                <td>
                                                    @if($row->status == 0)
                                                        <b style="padding: 0px 10px !important;color:#2874A6;">
                                                    @elseif($row->status == 1)
                                                        <b style="padding: 0px 10px !important;color:#1E8449;">
                                                    @elseif($row->status == 2)
                                                        <b style="padding: 0px 10px !important;color:#C70039;">
                                                    @elseif($row->status == 3)
                                                        <b style="padding: 0px 10px !important;color:#6C3483;">
                                                    @else
                                                        <b style="padding: 0px 10px !important;color:#FF5733;">
                                                    @endif
                                                    ...
                                                    <br>
                                                    {{($row->sevirity == 1)?'Normal':(($row->sevirity == 2)?'Urgent':'Very Urgent')}}
                                                </td>
                                                <td style="padding: 0px 10px !important;">{{date('d-m-Y H:i', strtotime($row->created_at))}}<br>{{$row->updated_by}}</td>
                                                ...
                                                <td style="padding: 2px 17px !important;">
                                                    ...
                                                    @endif 
                                                    @if($row->status == 0)
                                                        <a href="/eventRequisitions/{{$row->id}}/deactivate" onclick="return confirm('Are you sure to Delete Requisition....?')">
