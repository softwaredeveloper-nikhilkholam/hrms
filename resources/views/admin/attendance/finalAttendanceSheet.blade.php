...
@push('styles')
<style>
    /* All CSS rules have been moved to inline styles as requested. */
</style>
@endpush
    ...
    <div class="container-fluid">
        ...
        <div class="page-header d-xl-flex d-block">
            <div class="page-leftheader">
                <h4 class="page-title" style="font-weight: 600;">Final Attendance Sheet</h4>
            </div>
            <div class="page-rightheader">
                <a class="btn btn-primary" href="/empAttendances/confirmSheetList">Confirm Sheet List</a>
                <a class="btn btn-success" href="/empAttendances/finalAttendanceSheet">Final Sheet</a>
            </div>
        </div>
        <div class="card" style="box-shadow: 0 4px 15px rgba(0,0,0,0.07); border: none; border-radius: 10px; margin-bottom: 1.5rem;">
            ...
            <div class="card-body">
                {!! Form::open(['url' => route('admin.finalAttendanceSheet'), 'method' => 'GET']) !!}
                <div class="row align-items-end">
                    ...
                    <div class="col-md-2 form-group">
                        ...
                        {{ Form::select('branchId', $branches, $branchId ?? null, ['placeholder'=>'Select Branch','class'=>'form-control', 'required']) }}
                    </div>
                    <div class="col-md-2 form-group">
                        <label>Section <span class="text-danger">*</span></label>
                        {{ Form::select('section', ['Teaching'=>'Teaching', 'Non Teaching'=>'Non Teaching'], $section ?? null, ['placeholder'=>'All Sections','class'=>'form-control', 'required'])}}
                    </div>
                    <div class="col-md-3 form-group"></div>
                </div>
                <hr>
                <div class="row align-items-end mt-3">
                    ...
                    <div class="col-md-3 form-group">
                        <label><h3>Just search for the employee only</h3></label>
                        <input type="text" class="form-control" name="search_query" value="{{ $searchQuery ?? '' }}" placeholder="Search by Employee Name or Code...">
                    </div>
            ...
            <div class="card" style="box-shadow: 0 4px 15px rgba(0,0,0,0.07); border: none; border-radius: 10px; margin-bottom: 1.5rem;">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        @if($userType == '51' || $userType == '501')
                            {!! Form::open(['url' => route('admin.attendance.confirmation'), 'method' => 'POST']) !!} 
                        @endif
                        @if($userType == '61')
                            {!! Form::open(['url' => route('admin.attendance.process-and-save'), 'method' => 'POST']) !!} 
                        @endif
                            <input type="hidden" value="{{ $finalMonth ?? '' }}" name="fMonth">
                            <input type="hidden" value="{{ $branchId ?? '' }}" name="fBranchId">
                            <input type="hidden" value="{{ $section ?? '' }}" name="fSection">
                            <input type="hidden" value="{{ $searchQuery ?? '' }}" name="fSearch_query">
                            <div class="d-flex justify-content-between align-items-center mt-6">
                                <div>@if($attendances->hasPages()) {{ $attendances->appends(request()->query())->links() }} @endif</div>
                                <div>
                                    @if($userType == '51' || $userType == '501'  || $userType == '61')
                                        @if($attendanceConfStatus == 0)
                                            <button type="submit" class="btn btn-primary"><i class="fa fa-check-circle mr-2"></i>Confirm Page</button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-bg" disabled><i class="fa fa-lock mr-2"></i>Sheet Confirmed</button>
                                        @endif
                                    @endif
                                    <a href="{{ route('admin.attendance.export', request()->query()) }}" class="btn btn-outline-success">
