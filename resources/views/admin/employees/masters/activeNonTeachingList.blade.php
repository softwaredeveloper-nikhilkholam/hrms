        ...
        <div class="side-app">
            <div class="page-header d-xl-flex d-block">
                <div class="page-leftheader">
                     <h4 class="page-title"><i class="fas fa-users"></i> Non Teaching Employees</h4>
                </div>
                <div class="page-rightheader ml-auto">
                    <a href="/employees/create" class="btn btn-primary"><i class="fas fa-plus mr-2"></i>Add New Employee</a>
                    <a href="/employees/nonTeachingEmps" class="btn btn-danger">Non-Teaching</a>
                            ...
                            @if(isset($employees) && count($employees) >= 1)
                                <div class="table-responsive">
                                    <table class="table table-hover card-table table-vcenter text-nowrap mb-0" id="example">
                                        <thead>
                                            <tr>
                                                <th><i class="fas fa-check-circle"></i>Verify</th>
                                                ...
                                                <tr>
                                                    <td class="text-center">
                                                        @if($employee->verifyStatus == 1)
                                                            <i class="fas fa-check-circle verify-icon verify-success" data-toggle="tooltip" title="Verified"></i>
                                                        @else
                                                            <i class="fas fa-times-circle verify-icon verify-fail" data-toggle="tooltip" title="Not Verified"></i>
                                                        @endif
                                                    </td>
                                                    <td class="text-center"><span class="font-weight-bold">{{$employee->empCode}}</span></td>
                                                    <td class="text-center">
                                                            ...
                                                            <div class="employee-info">
                                                                <h6 class="employee-name">
                                                                    ...
                                                                    @if($employee->lastDate != '')
                                                                        <span class="status-badge status-left ml-2">Left</span>
                                                                    @endif
                                                                </h6>
                                                            </div>
                                                    ...
                                                    <td class="text-center">{{$employee->designationName}}</td>
                                                    <td class="text-center">{{$employee->departmentName}}</td>
                                                    <td class="text-center">{{$employee->branchName}}</td>
                                                    @if($userType == '61')
                                                        <td class="text-center">{{($employee->salaryScale == 0)?'₹0':'₹'.$util->numberFormat($employee->salaryScale)}}</td>
                                                    @endif
                                                    <td class="text-center">{{$employee->organisation}}</td>
                                                    <td class="text-center">{{date('h:i A', strtotime($employee->startTime))}} - {{date('h:i A', strtotime($employee->endTime))}}</td>
